	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		Draggable.create(".picture-history .pic-frame", {
			...
			bounds: document.querySelector(
				".picture-container.picture-history"
			),
			zIndexBoost: false
		})
		...
		Draggable.create(".picture-speaking .pic-frame", {
			...
			edgeResistance: 0.65,
			bounds: document.querySelector(
				".picture-container.picture-speaking"
			),
			zIndexBoost: false
		})
		...
		const selectedWorkItems = document.querySelectorAll(
			".selected-work--item"
		)
		...
		const designToolingCards = document.querySelectorAll(
			".design-tooling-card"
		)
